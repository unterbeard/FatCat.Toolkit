	...
	where T : MongoObject
{
	...
	public async Task<T> Create(T item) { return await repository.Create(item); }

	public async Task<List<T>> Create(List<T> items) { return await repository.Create(items); }

	public async Task<T> Delete(T item) { return await repository.Delete(item); }

	public async Task<List<T>> Delete(List<T> items) { return await repository.Delete(items); }

	public async Task<List<T>> GetAll() { return await repository.GetAll(); }

	public async Task<List<T>> GetAllByFilter(Expression<Func<T, bool>> filter) { return await repository.GetAllByFilter(filter); }

	public async Task<T?> GetByFilter(Expression<Func<T, bool>> filter) { return await repository.GetByFilter(filter); }

	public async Task<T?> GetById(string id) { return await repository.GetById(id); }

	public async Task<T?> GetById(ObjectId id) { return await repository.GetById(id); }

	public async Task<T?> GetFirst() { return await repository.GetFirst(); }

	public async Task<T> GetFirstOrCreate() { return await repository.GetFirstOrCreate(); }

	public void SetUpItemNotInRepository() { A.CallTo(() => repository.GetById(A<string>._)).Returns(null as T); }

	public async Task<T> Update(T item) { return await repository.Update(item); }

	public async Task<List<T>> Update(List<T> items) { return await repository.Update(items); }
