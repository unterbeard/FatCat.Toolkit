...
public static class ConsoleLog
{
	...
	private static string? GetPrintableStackTrace(string spaces, Exception exception) => string.IsNullOrEmpty(exception.StackTrace) ? null : exception.StackTrace.Replace(Environment.NewLine, $"{spaces}{Environment.NewLine}                ");
	...
	private static void WriteExceptionForLevel(Exception exception, int level, [CallerMemberName] string memberName = "",
												[CallerFilePath] string sourceFilePath = "",
												[CallerLineNumber] int sourceLineNumber = 0)
	{
